...
import { PdfDataPer, PdfDataPermission, VerifyUserPermission } from "./types/global";
...
const transformPdfdata = (data: VerifyUserPermission[]): PdfDataPermission => {
  ...
  const groupedPermissionsWithPercentage = Object.entries(groupedPermissions).map(
    ([permission, object]) => {
      const contJustifyPermission = object.permissions.filter(
        ({ justifyPermission }) => justifyPermission
      ).length;
  ...
  switch (args[0]) {
    case "verify":
      ...
      const data = {
        ...
        owasp: transformPdfdata(userPermissions || [])
      };
